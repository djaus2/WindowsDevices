...
namespace DeviceWatcher
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        async void DisplayDeviceInterfaceArray()
        {
            ...
            var myInterfaces2 = from p in myInterfaces1 where filter(p) select p;

            //Apply set by name if selected
            if (chkSortOnName.IsChecked == true)
            {
                 myInterfaces2 = from p in myInterfaces1 where filter(p) orderby p.Name select p ;
            }
        ...
        bool filter(DeviceInformation deviceInterface)
        {
